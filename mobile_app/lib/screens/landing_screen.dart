...
class _LandingScreenState extends State<LandingScreen> {
  ...
  void _submitNumber() async {
    ...
    try {
      // Call the real banking API for login
      final result = await _bankingAPI.login(mobile);

      if (result["success"]) {
        // Generate OTP for verification (in real app, this would come from API)
        final otp = "123456"; // This should ideally come from the API response
        ...
        Navigator.pushNamed(context, '/OtpScreen',
            arguments: OtpScreenParams(
                mobileNumber: int.parse(mobile), otp: int.parse(otp)));
      } else {
        // Show error message
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
