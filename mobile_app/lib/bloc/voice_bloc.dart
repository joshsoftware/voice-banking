...
class Error extends VoiceState {
  final String message;
  Error(this.message);
}
...
class VoiceBloc extends Bloc<VoiceEvent, VoiceState> {
  ...
  VoiceBloc(this.repo) : super(Idle()) {
    on<StartListening>((e, emit) async {
      try {
        print("Voice Bloc: Starting listening...");
        await repo.start();
        print("Voice Bloc: Recording started successfully");
        emit(Listening());
      } catch (e) {
        print("Voice Bloc Error starting voice recording: $e");
        emit(Error("Failed to start recording: ${e.toString()}"));
    ...
    on<StopListening>((e, emit) async {
      try {
        print("Voice Bloc: Stopping listening...");
        emit(Transcribing());
        _currentLocale = e.locale; // Store current locale
        print("Voice Bloc: Starting transcription with locale: ${e.locale}");
        final data = await repo.stopAndTranscribe(locale: e.locale);
        print("Voice Bloc: Transcription completed");
        add(GotTranscript(data, e.locale));
      } catch (e) {
        print("Voice Bloc Error stopping voice recording: $e");
        emit(Error("Failed to stop recording: ${e.toString()}"));
        emit(Idle());
      }
    });

    on<Reset>((e, emit) {
      print("Voice Bloc: Resetting to idle state");
      emit(Idle());
    });
    ...
    on<GotTranscript>((e, emit) async {
      try {
        ...
        if (intentName == "recent_txn" && orchestratorData != null) {
          ...
          try {
            print("Voice Bloc: Speaking translated message: $translatedMessage");
            await tts.speak(translatedMessage, langCode: ttsLanguage);
            print("Voice Bloc: TTS completed successfully");
          } catch (e) {
            print("TTS Error: $e");
          }
        ...
        print("Voice Bloc Error in GotTranscript: $e");
        emit(Error("Failed to process voice command: ${e.toString()}"));
        emit(Idle());
      }
    });
  }
